...
- **KineDriver importer:** select an armature, then use `File > Import > FF7R Rebirth > KineDriver JSON`. It audits the selected KDI file internally and builds the driver layer in one operation. The audit graph is saved in the `.blend`; `Object > Retrilogy tools > KDI: Visualize Driver Graph` therefore opens it without asking for the KDI again. In Pose Mode, right-click a KDI-driven bone and choose **KDI: Focus Selected Bone in Graph** to show just the subgraph that drives it in any open node editor.
