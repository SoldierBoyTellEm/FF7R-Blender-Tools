...
def build_objects_from_component(
    ...
    root_coll = bpy.data.collections.new(base_name)
    ...
    for group_idx in range(num_groups):
        ...
        # Submesh records (length28 × 64 bytes) — vectorized: read all 16-int32 records at once.
        if submesh_rec_cnt:
        ...
        #   [4:16]  3× float32 px,py,pz
        #   [16:32] 4× float32 qx,qy,qz,qw
        if instance_cnt:
        ...
        # Triangle group records — 16 bytes each: skip 4, then base/ntri/dataoff (3× int32).
        if tri_group_cnt:
        ...
        for mt, inst_list in mesh_instances.items():
            ...
            #           split_norms.append(norms[mesh.loops[loop_idx].vertex_index])
            #   mesh.normals_split_custom_set(split_norms)
            # Since every loop sharing a vertex got that vertex's normal, the per-vertex
            # variant produces an identical result in one C call.
            ...
            if mat_name:
                ...
                # Single-material mesh — fill material_index with zeros via foreach_set.
